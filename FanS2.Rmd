...
p_checklist<-usable_data %>%  
  select(c(110:159)) # Selected only the partnered fantasy checklist 

s_checklist<-usable_data %>%  
  select(c(173:222)) # Selected only the solitary fantasy checklist 

# Partnered fantasy, count of all NAs
p_checklist %>% summarise_all(~ sum(is.na(.)))

p_checklist_no_na<-na.omit(p_checklist)

length(p_checklist$pfan.checklist_1)-length(p_checklist_no_na$pfan.checklist_1) # If we remove all rows with NA, then we remove 64 participants

# Solitary fantasy
s_checklist %>% summarise_all(~ sum(is.na(.)))

s_checklist_no_na<-na.omit(s_checklist)

length(s_checklist$sfan.checklist_1)-length(s_checklist_no_na$sfan.checklist_1) # If we remove all rows with NA, then we remove 58 participants
...
```{r}
mardia(s_checklist, na.rm = T)
```

These results show that the solitary checklist is also non-normal (especially the Q-Q plot).

## Calculating polychoric correlation matrices

Using the subsets I've created above, I will now create two polychoric correlation matrices, using the polychoric function from the psych package. I've decided to use the matrices that do not have NA values (na.omit), so the sample size overall is lower.

```{r}
# Partnered fantasy
p_check_poly<-polychoric(p_checklist_no_na, smooth = T, # Smoothing the data
                         global = F, # running the analysis using the local values
                         na.rm = T # removing the missing data
                         )

p_check_matrix<-p_check_poly$rho # Selecting the rho values, so that we have a correlation matrix

# Solitary fantasy
s_check_poly<-polychoric(s_checklist_no_na, smooth = T, # Smoothing the data
                         global = F, # running the analysis using the local values
                         na.rm = T # removing the missing data
                         )

s_check_matrix<-s_check_poly$rho # Selecting the rho values, so that we have a correlation matrix
```
...
Using the subsets I've created above, I will now create two polychoric correlation matrices, using the polychoric function from the psych package. 

```{r}
# Partnered fantasy
fa.parallel(p_check_matrix, n.obs=499, fm = "uls", n.iter = 20, cor = "poly")
```
## Solitary fantasy

```{r}
# Partnered fantasy
fa.parallel(s_check_matrix, n.obs=505, fm = "uls", n.iter = 20, cor = "poly")
```