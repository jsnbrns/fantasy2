...
```{r}
# Partnered fantasy
p_check_poly_psych<-polychoric(p_checklist, smooth = T, # Smoothing the data
...
# Solitary fantasy
s_check_poly_psych<-polychoric(s_checklist, smooth = T, # Smoothing the data
                         global = F, # running the analysis using the local values
                         na.rm = T # removing the missing data
                         )
...
```{r}
# Partnered fantasy
p_check_poly_lavaan<-lavCor(p_checklist, ordered = names(p_checklist), # This tells it that the variables are ordinal
                            missing = "pairwise", #This does pairwise deletion 
                            output = "cor" #This outputs a correlation matrix
                            )
#N
563-24
same_p<-p_check_poly_lavaan == p_check_matrix_psych
length(same_p[same_p== TRUE])
## The matrices are not the same


# Solitary fantasy
s_check_poly_lavaan<-lavCor(s_checklist, ordered = names(s_checklist), # This tells it that the variables are ordinal
                            missing = "pairwise", #This does pairwise deletion 
                            output = "cor" #This outputs a correlation matrix
                            )
same_s<-s_check_poly_lavaan == s_check_matrix_psych
length(same_s[same_s== TRUE])
## The matrices are not the same
...
```{r}
# Partnered fantasy
fa.parallel(p_check_poly_lavaan, n.obs=539, fm = "uls", n.iter = 20, cor = "poly")
```
## Solitary fantasy

```{r}
# Partnered fantasy
fa.parallel(s_check_poly_lavaan, n.obs=545, fm = "uls", n.iter = 20, cor = "poly")
```